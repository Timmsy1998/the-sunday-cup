<template>
  <div class="container my-5 d-flex aligns-items-center justify-content-center">
    ...
    <form
      @submit.prevent="
        handleSubmit(username, email, password, confirmPassword, summonerName, isVerified)
      "
      ...
      <div class="row">
        <div class="form-group col-12 mb-3">
          ...
          <input
            ...
            required
          />
          <div v-if="errors.summonerName" class="text-danger">
            {{ errors.summonerName }}
          </div>
        </div>
      </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body" v-if="showVerifyButton">
            <div v-if="errors.verification" class="text-danger mb-3">
              {{ errors.verification }}
            </div>
            <div v-if="!showVerifyButton" class="text-success mb-3">
              Summoner has been verified! Please click close on this box!
            </div>
...
export default {
  ...
  setup() {
    ...
    watch(summonerName, (summonerName) => {
      ...
      try {
        ...
        await axios.post("/api/register", data).then((response) => {
          if (response.data.status === "success") {
            ...
            successMessage =
              "Registration submitted successfully... please wait for redirection. If it does not redirect after 10 seconds, please manually refresh the page";
